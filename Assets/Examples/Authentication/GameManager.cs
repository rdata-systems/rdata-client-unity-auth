...
namespace RData.Authentication.Examples
{
    public class GameManager : MonoBehaviour
    {
        ...
        private const string kStatusSendingRegistrationRequest = "Sending registration request...";
        ...
        private const string kStatusRegistrationFailed = "Registration failed";
        ...
        private IEnumerator OnLoginButtonClickedCoro()
        {
            ...
            string login = (usernameInputField.text != null ? usernameInputField.text : emailInputField.text);
        ...
        {
            StartCoroutine(OnRegisterButtonClickedCoro());
        }

        public IEnumerator OnRegisterButtonClickedCoro()
        {
            SetFormInteractable(false);
            SetStatus(kStatusSendingRegistrationRequest);
            yield return StartCoroutine(_authenticationClient.Register(usernameInputField.text, emailInputField.text, passwordInputField.text));

            // Check if any errors happened during authentication
            if (_authenticationClient.HasError)
            {
                SetError(_authenticationClient.LastError);
                SetStatus(kStatusRegistrationFailed);
                SetFormInteractable(true);
                yield break;
            }

            SetStatus(kStatusRegistrationSuccessful);
            SetFormInteractable(true);
        }
